...
## upload to youtube
 TEXT=$(date -d $(echo $FILE|cut -d "." -f 1) +"%B %d %Y")
 /usr/local/bin/youtube-upload -m andys@florapdx.com -p $PASS -t "Portland Oregon sunrise $TEXT" -c "Entertainment" -d "https://github.com/andyseubert/sunrise" $FILE.avi
else 
 echo "boo hoo"
